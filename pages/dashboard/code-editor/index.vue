...
<template>
  <div class="w-full mx-auto my-16 space-y-[30px] max-w-7xl">
    <div class="flex justify-between items-center">
      ...
      <UButton class="h-10" color="white" @click="newCode()" :disabled="loadingState.create"
        ...
        <div class="col-span-12" v-if="data?.length === 0">
          <div class="relative flex justify-center items-center h-[300px]">
            <div class="flex flex-col justify-center items-center relative z-10 gap-2">
              ...
              <h2 class="text-2xl font-bold">Code is empty, <span class="underline cursor-pointer"
                  @click="newCode()">create</span> a new one</h2>
            </div>
          ...
          <UCard>
            <div class="flex justify-between items-center">
              <div>
                ...
                <p class="text-xs opacity-50">updated {{ item.updated_at ? formatDistanceToNow(parseISO(item.updated_at))
                  : ''}} ago</p>
              </div>
              <div class="flex gap-2">
                ...
                <UButton class="h-8 w-8 flex items-center justify-center" color="red" @click="removeCode(item.id)"
                  :disabled="loadingState.remove && loadingState.removeOnId == item.id"
                  :loading="loadingState.remove && loadingState.removeOnId == item.id">
                  <UIcon name="i-lucide-trash" />
                </UButton>
              </div>
            </div>
