...
<template>
  <div class="flex max-h-full">
    <div
      class="fixed z-10 p-2 bg-stone-900 left-2 top-2 rounded-lg w-10 h-10 flex md:hidden items-center justify-center"
      @click="menuOpen = !menuOpen">
      <UIcon name="i-lucide-menu"></UIcon>
    </div>

    <div class="menu-overlay fixed top-0 left-0 w-full h-full bg-black opacity-20 z-40 block md:hidden" v-if="menuOpen"
    ...
    fixed z-50 bg-white dark:bg-stone-950 md:bg-transparent" id="menu-admin-top" v-if="menuOpen">
      <div class="py-2 flex flex-col justify-between h-[100vh] bg-background">
        <div class="flex flex-col gap-3">
          <NuxtLink to="/dashboard">
            <div class="flex items-center justify-start gap-2">
              <p class="text-lg leading-tight m-4">
                Tulongeni
              </p>
            </div>
          </NuxtLink>

          <UVerticalNavigation :links="linksTopNav">
        ...
        <div class="flex flex-col">
          <UVerticalNavigation :links="linksBottomNav">
          ...
          <div class="flex items-center justify-between p-3">
            <div class="flex items-center gap-2">
              <UAvatar size="sm" :alt="auth?.email" :src="auth?.user_metadata.avatar_url ?? null" />
              <p class="text-gray-400 font-bold text-sm" v-if="auth?.email">{{ auth.email.length > 12 ?
                auth.email.slice(0, 12) +
                '...' : auth.email }}</p>
            </div>
            <UButton color="red" variant="ghost" icon="i-lucide-log-out" @click="signOut">
            </UButton>
          </div>
        </div>
      </div>
